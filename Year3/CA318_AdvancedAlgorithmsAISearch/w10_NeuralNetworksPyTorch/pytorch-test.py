...
#
def train_neural_network(neural_network, train_loader, learning_rate):
   optimizer = optim.SGD(neural_network.parameters(), lr = learning_rate)
   time0 = time()
   loss_function = nn.NLLLoss()

   # Weights can be changed in the neural network as the number of epochs changes
   # Curve may go from underfitting to optimal to overfitting the curve
   epochs = 15
...
def main():
   ...
   # this influences the size of the jumps that leads to the global optimum
   learning_rate = 0.1

   if os.path.exists('models/' + 'nn128_64_' + str(learning_rate) + '.nn'):
      neural_network.load_state_dict(torch.load('models/' + 'nn128_64_' + str(learning_rate) + '.nn'))
   else:
      train_neural_network(neural_network, train_loader, learning_rate)
   
      torch.save(neural_network.state_dict(), 'models/' + 'nn128_64_' + str(learning_rate) + '.nn')
