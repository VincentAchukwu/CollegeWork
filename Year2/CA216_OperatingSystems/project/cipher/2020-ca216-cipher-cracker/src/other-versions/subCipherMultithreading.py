import random, time, string, sys, os, queue, threading
...
core_count = threading.cpu_count()     # theoretical maximum number of processes we can run at any given moment
...
class SubCipher(object):
    ...
    def decodeAlgo(self, cipher, match):
        ...
        while True:
            ...
            if i > 20:  # program can stop if there are no better scores after 20 iterations
                return self.save_key_dec(output, curr_best_key)    # write decrypted text and key to their corresponding files in the Substitution Cipher directory
    ...
    def multiprocess(self, i, cipher, parentkey, parentscore, match, curr_best_key, curr_best_score):
        ...
        for _ in range(core_count):
            p = threading.Thread(target=self.test_curr_key, args=(cipher, parentkey, parentscore, match)).start()

        for _ in range(core_count):
            i += 1
            parentkey, parentscore = self.q.get()

            # print("{} : {}".format(i, parentscore))   # used this to print current iteration and parentscore
            if parentscore > curr_best_score:
                ...
                # converting all uppercase to lowercase and vice versa
                output = ""
                for letter in plain:
