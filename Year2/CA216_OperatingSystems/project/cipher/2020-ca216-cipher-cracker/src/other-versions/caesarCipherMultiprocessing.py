...
import string, collections, time, sys, os, multiprocessing
...
class caesarCypher(object):
    ...
    def __init__(self, q):
        ...
        self.q = q                                   # queue to add decrypted words
    ...
    def save_key_dec(self, dec_lst):
        ...
        with open(os.path.join(caesar_dir, "{}-decrypted.txt".format(file[:-4])), "w") as d:
            # maybe while q.not empty write q.get()
            # while not self.q.empty():
            d.write("\n".join(dec_lst))     # sentences join back together to appear in same structure as encrypted text file (for output elegancy..)


        return "{0}-decrypted.txt and {0}-key.txt written in caesar-cipher Cipher directory. Exiting.".format(file[:-4])
...
def main():
    ...
    cc = caesarCypher(q)                # create instance of class and add q to it

    cipher = cc.load_enc_File(caesar_dir)   # load cipher text from caesar-cipher directory

    key = cc.keygenerator()             # calls method to generate key
    ...
    for sentence in cipher:
        for enc_word in sentence.split():
            p = multiprocessing.Process(target=cc.decrypt, args=(enc_word, key))
            dec_Processes.append(p)
            p.start()
        # in the end, making decrypted file be the same output format as the encrypted file text.
        dec = []                        # decrypted words stored in list
        while not cc.q.empty():         # extract words from queue and append to dec
            dec.append(cc.q.get())
        dec_lst.append(" ".join(dec))   # then append dec to dec_lst (probably not best way of doing it)

    for p in dec_Processes:
        p.join()
