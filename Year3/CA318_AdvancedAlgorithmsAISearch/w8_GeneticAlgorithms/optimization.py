...
def mutate(gene, space, max_step):
    ...
    if gene[spot] < space[spot].minimum: gene[spot] = space[spot].minimum
    if gene[spot] > space[spot].maximum: gene[spot] = space[spot].maximum
...
def genetic_algorithm(space, cost, elite_fraction = 1/5, pop_size = 50, mutation_rate = 0.6, max_step = 100, max_iterations = 100):
    # Create pop_size random genes + add to poulation
    population = []
